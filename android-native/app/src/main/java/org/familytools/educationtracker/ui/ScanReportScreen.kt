...
fun ScanReportScreen(viewModel: AcademicRecordsViewModel, onBack: () -> Unit) {
    ...
    suspend fun runOcr(uri: Uri) {
        ...
        try {
            ...
            val extracted = OcrService.parseReportText(text)
            if (extracted.isNotEmpty()) {
                ...
                status = "Extracted ${extracted.size} subject row(s) — please verify."
            } else {
                status = "Couldn't automatically parse rows — please enter marks manually."
            }
        } catch (e: Exception) {
